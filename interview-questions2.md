...
外部的 `valA` 在 `main 的栈` 上有一份内存，同时 `block 的栈` 上也会拷贝 valA 的一份内存； 由于函数调用的内存机制，valA 在不同的栈上，作用域不同，所以是不能修改的。 并且如果是 `对象类型` 的，在 `block` 内部是 `const` 的，不能修改。
...
​			[__block修饰符探究](http://aevit.xyz/2017/01/09/iOS-__block/)（参考这个吧，这个是从作用域角度去解释__block，个人感觉比较易懂）
...
2. 抽象工厂（Abstract Factory）
   ...
   参考：[iOS 三种工厂模式(简单工厂模式、工厂模式、抽象工厂模式)](https://www.jianshu.com/p/847af218b1f0)
